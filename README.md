...
REEA-371 keep-warm: `.github/workflows/keep-warm.yml` pings `GET /api/health` on two anchors per hour (`7,37 * * * *`, max gap ~30 min) so the Hobby alias never sits idle long enough to scale down — first request after idle answers 200 JSON in <5s instead of a ~30s cold boot (observed 503 after ~29.5s, immediate retry instant 200). Curl-only job (~10s/run, ~250 min/month typical worst-case ~700) rides inside the ~2,000 min/month Hobby Actions allowance alongside the smoke checks; scheduled pre-warm was chosen over `minInstances` because a permanently held instance burns Hobby's included function usage just to idle, and the ping doubles as the funnel liveness check the Vercel cron already carries. Rollback: delete the cron lines (or the file) and push — alias serving is unaffected; the smoke-check anchors remain as fallback heartbeat.

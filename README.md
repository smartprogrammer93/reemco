...
REEA-371 keep-warm: `.github/workflows/keep-warm.yml` pings `GET /api/health` every 5 min (step cron `2-59/5 * * * *`, off the :00 spike) plus once on each push to `main`, so the Hobby alias never cools past its ~5 min fast-answer window (measured 04:20–04:41Z: ~2s answer within minutes of recent traffic, ~30s after ~6 min idle; REEA-369 cache-first replay then makes the immediate retry instant). Scheduled ticks skip occasionally in this environment (REEA-71) and self-heal on the next step ≤10 min later; push triggers are the proven-reliable warm path for fresh artifacts. Curl-only job (~8s/run, ~1,150 min/month ceiling) plus smoke checks (~600) and push pairs stays inside the ~2,000 min/month Hobby Actions allowance; scheduled pre-warm chosen over `minInstances` because a permanently held instance burns Hobby's included function usage just to idle, and the ping doubles as the funnel liveness check the Vercel cron already carries. Rollback: delete the cron line and the `push:` trigger (or the whole file) and push — alias serving is unaffected; smoke-check anchors remain as fallback heartbeat.
